...
- 只有验证通过的 `main` push 才部署到 Cloudflare；
- 如果配置了生产 URL，还会执行 production verification。

启用生产部署前，需要在 GitHub repository secrets 中配置：
...
Cloudflare API token 应尽量采用最小权限范围。

## 按需格式

`.github/workflows/build-publication.yml` 只通过 `workflow_dispatch` 手工启动。

用户明确选择 EPUB、PDF、DOCX 或 LaTeX 中的一种格式。生成文件上传为 GitHub Actions artifact。
...
4. 确定后填写 canonical production URL；
5. 替换示例 QMD；
6. 添加 bibliography 和原始 assets；
7. 根据真实项目需要，在 deploy 前增加更严格的 source/output validation。
