...
5. 面对**新项目**时，读取 [`docs/PER_PROJECT_GITHUB_CLOUDFLARE_SETUP.zh-CN.md`](docs/PER_PROJECT_GITHUB_CLOUDFLARE_SETUP.zh-CN.md)、[`docs/CI_COST_POLICY.zh-CN.md`](docs/CI_COST_POLICY.zh-CN.md)、[`docs/CLOUDFLARE_GITHUB_AUTHORIZATION.zh-CN.md`](docs/CLOUDFLARE_GITHUB_AUTHORIZATION.zh-CN.md) 与 Starter 的 Project Provisioning Contract；默认采用 `workers-builds-native` + `private-project-quota-saver` 的每项目一次引导式配置，只有项目显式选择高级 Profile 时才读取 External-CI / Secret Broker 契约
...
对于包含原创或未发布内容的新项目，安全默认值是：canonical 源仓库 private，同时 Continuous Web 保持 restricted + authenticated。当通过完整 Inquiry Publishing Stack 采用时，默认组合是完整 PPF + 完整 AHICP + Vault Interface；精简 profile 必须由人类明确选择。默认允许每个项目一次短而明确的 GitHub / Cloudflare 人工 bootstrap，并采用 `private-project-quota-saver`；不能因为 public framework repo 使用完整 CI，就自动把同样的 Actions footprint 复制进 private 项目。
...
Private downstream 项目不得把 GitHub Actions 当作迭代调试器。Agent 必须先批量完成相关修改、运行所有可用 Agent-side preflight、检查完整 diff，再最多触发一次预定的薄 CI。CI 失败时先读完整 failure set、批量修复、重新 preflight，并优先只 rerun failed job / failed workflow，不得把已经成功的重型工作一起重跑。Content-only 改动不能为了“再确认一下”而制造 Actions run。
