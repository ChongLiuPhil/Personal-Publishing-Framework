...
不得要求人类把密码、token、私钥、恢复码或其他秘密粘贴到聊天。Agent-Provisioned External CI 的项目 deployment token 必须经 trusted secret broker 直接进入 GitHub Actions secrets；模型只能看到非秘密安装状态。若当前 Cloudflare UI 与已有 runbook 不同，应核对当前 UI 或官方文档，不得猜测。
