...
For ordinary new projects, Cloudflare Workers Static Assets delivered through Workers Builds Native is the preferred provisioning path: keep the personal-account GitHub repository private, allow one short human-assisted repository connection, protect the Worker with Access, and verify a second push deploys automatically without renewed authorization. Private downstream projects also default to `private-project-quota-saver`: no automatic GitHub Actions for content-only changes or main pushes, one lightweight configuration-PR contract gate, manual heavy workflows, and Cloudflare as the single automatic production Web builder. GitHub Actions external CI with a project-scoped Worker credential remains an optional advanced profile. GitHub remains the canonical source/version-control provider. Cloudflare Pages remains supported for existing projects and is not migrated automatically. The verified canonical framework site now uses https://inquirystack.philohub.workers.dev/; the former framework GitHub Pages sites have been retired. A downstream publication may select Pages, Workers, or another supported provider according to its actual runtime and publication requirements.
...
Use [`CONTINUOUS_WEB_CLOUDFLARE.md`](CONTINUOUS_WEB_CLOUDFLARE.md) for the detailed Cloudflare publication contract, [`PER_PROJECT_GITHUB_CLOUDFLARE_SETUP.md`](PER_PROJECT_GITHUB_CLOUDFLARE_SETUP.md) for the ordinary per-project setup, and [`CI_COST_POLICY.md`](CI_COST_POLICY.md) for private-repository Actions/build-budget behavior. [`AGENT_PROVISIONED_EXTERNAL_CI.md`](AGENT_PROVISIONED_EXTERNAL_CI.md) documents the optional advanced profile, while [`GITHUB_CLOUDFLARE_INTEGRATION.md`](GITHUB_CLOUDFLARE_INTEGRATION.md) retains reusable infrastructure state and reconciliation boundaries. The [security profiles](CLOUDFLARE_SECURITY_PROFILES.md) describe deployment-credential trade-offs; deployment identity and reader access are separate decisions.
