...
## Human-operation and write-back rule
...
Before the human opens a provider UI, the Agent must persist the relevant step as `waiting-human`. After the human returns, the Agent verifies actual provider state and only then marks the step completed.
...
Current provider references for these UI paths: [GitHub repository creation](https://docs.github.com/en/repositories/creating-and-managing-repositories/creating-a-new-repository), [Cloudflare Git integration](https://developers.cloudflare.com/workers/ci-cd/builds/git-integration/), and [Cloudflare Worker Access](https://developers.cloudflare.com/workers/configuration/cloudflare-access/).
...
Cloudflare Access requires Zero Trust to be enabled on the account. If this is the first protected Worker and Zero Trust is not yet enabled, open the Cloudflare dashboard → **Zero Trust** and complete the one-time onboarding. If the project intends to remain on a free plan, confirm the UI still identifies the intended free/$0 plan before accepting any billing change; if the actual UI requires a paid upgrade or other material billing decision, stop and return that decision to the human. Once Zero Trust is enabled, write back `cloudflare.zero_trust: verified`. This is an account prerequisite later projects reuse, not a per-project authorization.
