...
## 0. 人工操作与 Write-Back 规则

如果项目采用完整 Starter 栈，人工配置状态由 `project-bootstrap-state.yaml` 与 AHICP Working Memory 持久记录。

Agent 在要求人类打开 Provider UI 前，应先把对应步骤写成 `waiting-human`；人类返回后，Agent 先验证 actual Provider state，再把步骤写成 completed。
...
Cloudflare Access 要求账户先启用 Zero Trust。如果这是第一个受保护 Worker，而账户尚未启用：进入 Cloudflare Dashboard → **Zero Trust**；在 onboarding 页面选择 team name；按预期计划完成 onboarding。如果项目政策是继续使用 Free plan，在接受任何 billing 变化以前先确认界面仍明确显示 Free / $0 选项。Cloudflare 当前可能即使 Free plan 也要求填写 payment details；如果实际界面要求付费升级或不同的 billing 决定，停止并把决定交还人类。Zero Trust organization 建立后，写回 `cloudflare.zero_trust: verified`。这个账户级前置条件后续项目可以直接复用。
