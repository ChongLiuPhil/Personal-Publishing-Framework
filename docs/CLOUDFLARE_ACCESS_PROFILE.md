...
Cloudflare Access is the infrastructure authentication layer. Application code does not add password/session authentication unless the project implements actual application user accounts. PPF does not define a shared-password publishing mode.
...
Current Cloudflare Access policy selectors are identity/policy oriented (for example email, login method, group, device posture, service token). Choose an Access policy that matches the approved reader audience; do not replace it with an application password.
...
Current Cloudflare Workers Access supports account-wide, Worker-level, and hostname/path scopes.
...
## 9. Current official Access references

Reviewed against current Cloudflare documentation on 2026-09-23:

- Cloudflare Workers — Cloudflare Access:
  https://developers.cloudflare.com/workers/configuration/cloudflare-access/
- Cloudflare One — Require Access protection:
